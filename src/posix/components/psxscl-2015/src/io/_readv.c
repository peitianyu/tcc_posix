...
ssize_t __sys_readv(int fd, const struct iovec * iov, int iovcnt)
{
	...
	for (readv_bytes=iovc=0; iovc<iovcnt && !iosb.status; iovc++) {
		iov_buf   = (char *)iov[iovc].iov_base;
		iov_bytes = iov[iovc].iov_len;

		ofd->info.iostatus = __iovtbl[ofd->info.fdtype].read(
			ofd->info.hfile,
			ofd->info.hevent,
			0,0,
			&iosb,
		...
		if (!iosb.status)
			readv_bytes += iosb.info;
	}

	iosb.info = readv_bytes ? readv_bytes : iosb.info;
