...
static int32_t __fastcall __psx_iofn_devzero_open_next(
...
int32_t	__stdcall __psx_iofn_devzero_read(
	void *			hfile,
	void *			hevent,
	...
	uint32_t *		key)
{
	__ntapi->tt_generic_memset(buffer,0,bytes);
...
int32_t __stdcall __psx_iofn_devzero_write(
	void *			hfile,
	void *			hevent,
	nt_io_apc_routine *	apc_routine,
	void * 			apc_context,
...
static int32_t __stdcall __psx_iofn_devzero_open_parent_directory(
	void **		hparent,
	void *		hdir,
	uintptr_t *	buffer,
...
void __psx_iofn_devzero_init(struct __iovtbl * iovtbl)
{
	iovtbl->open_next	= __psx_iofn_devzero_open_next;
	...
	iovtbl->read		= __psx_iofn_devzero_read;
	iovtbl->write		= __psx_iofn_devzero_write;
	...
	iovtbl->open_logical_parent	= __psx_iofn_devzero_open_parent_directory;
	iovtbl->open_physical_parent	= __psx_iofn_devzero_open_parent_directory;
}
