...
static int32_t __fastcall __psx_iofn_fsfile_open_next(
	struct __path_info *	path_info,
	int32_t			index)
{
	if (path_info->fexflags & PSX_PATH_INTERNAL_CALL)
		return __iovtbl[PSX_FD_OS_FS_DIR].open_next(path_info,index);
	else
		return NT_STATUS_NOT_A_DIRECTORY;
}

int32_t __fastcall __psx_iofn_fsfile_fsync(struct __ofd * ofd, nt_iosb * iosb)
{
	return __ntapi->zw_flush_buffers_file(ofd->info.hfile,iosb);
}

void __psx_iofn_fsfile_init(struct __iovtbl * iovtbl)
{
	iovtbl->open_next	= __psx_iofn_fsfile_open_next;
	...
	iovtbl->fsync		= __psx_iofn_fsfile_fsync;
