...
static int32_t __fastcall __psx_iofn_procself_open_next(
...
static int32_t __stdcall __psx_iofn_procself_open_parent_directory(
	void **		hparent,
	void *		hdir,
	uintptr_t *	buffer,
...
{
	*hparent = 0;
	*type = PSX_FD_OS_PROCFS;
	return NT_STATUS_SUCCESS;
}


void __psx_iofn_procself_init(struct __iovtbl * iovtbl)
{
	iovtbl->open_next	= __psx_iofn_procself_open_next;
	...
	iovtbl->open_logical_parent	= __psx_iofn_procself_open_parent_directory;
	iovtbl->open_physical_parent	= __psx_iofn_procself_open_parent_directory;
}
