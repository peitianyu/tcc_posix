...
static int32_t __fastcall __psx_iofn_devtty_open_next(
...
static int32_t __stdcall __psx_iofn_devtty_open_parent_directory(
	void **		hparent,
	void *		hdir,
	uintptr_t *	buffer,
...
static int32_t __fastcall __psx_iofn_devtty_stat(struct __psx_tlca * tlca, struct __ofd * ofd, struct __stat * stat)
{
	__ntapi->tt_generic_memset(stat,0,sizeof(*stat));
	return NT_STATUS_SUCCESS;
}

static int32_t __fastcall __psx_iofn_devtty_unlink(struct __ofd * ofd, uint32_t flags)
{
	return NT_STATUS_ACCESS_DENIED;
}

void __psx_iofn_devtty_init(struct __iovtbl * iovtbl)
{
	iovtbl->open_next	= __psx_iofn_devtty_open_next;
	...
	iovtbl->stat		= __psx_iofn_devtty_stat;
	iovtbl->unlink		= __psx_iofn_devtty_unlink;
	...
	iovtbl->open_logical_parent	= __psx_iofn_devtty_open_parent_directory;
	iovtbl->open_physical_parent	= __psx_iofn_devtty_open_parent_directory;
}
