...
故 `-b`/`-bt` 建议走独立 exe 路径。进一步定位 (指令级): 空 main 与**仅栈/堆**边界访问的
`-run -b` 均正常 (exit 0); **一旦存在全局/static 数组**即在其边界访问路径以
`mov rcx,[rcx]` (`48 8b 09`) 坏指针解引用崩溃, 且 `__bound_init` 的全局登记循环
入口从未执行到 —— 指向 `.bounds` 段在 `-run` 内存链接中未按运行时基址正确重定位,
导致全局区域登记/表遍历出错, 属 tcc `-run` 链接器对 bcheck 运行时对象的深水区缺陷,
需专项攻坚(可能是 `-run` 对 `bounds_section` 的重定位/`rc->bounds_start` 取值)。
`-run -bt` 正常。
