...
### 4.2 model 泛型 (t031-t032c: 基础/函数/常量参数/边界组合)
...
支持多类型参数 (Pair2(T,U)/Tri(T,U,V))、嵌套实例化 (`Array(Box(int))`, 3 层
`Wrap(Array(Box(int)))`)、**泛型递归** (`struct Node(T) { T v; struct Node(T)
*next; }` — struct 关键字自引用与裸 `Node(T)` 均可, 自/互引用经实例化中占位
缓存破环)、常量参数 (`model (int N,T) T f(T a[N])` 编译期归一化数值实参, 内部名
按数值而非文本签名确定; 常量实参可传嵌套实例 `Grid→Mat(T,N,2)`)。
边界组合见 t032c (2026-08-25 收敛: 多类型×嵌套×常量 + 缓存一致性, tcc 编译与
脱糖 clang 产物数字一致)。**已知限制**: 模板名与成员/参数名不得重复 (实例化
替换会冲突); 函数模板实参必须是类型 (非类型模板参数不支持); 匿名 bitfield
不入反射。
