...
int main(void) {
    ...
    /* ---------------- Stack ---------------- */
    {
        STL_Stack(int) s; stl_stack_init(int)(&s, ar);
        CHECK(stl_stack_empty(int)(&s));
        for (int i = 0; i < 40; i++) stl_stack_push(int)(&s, i * 3);
        CHECK(stl_stack_size(int)(&s) == 40);
        CHECK(stl_stack_top(int)(&s) == 117);       /* 39*3 */
        for (int i = 39; i >= 10; i--) { CHECK(stl_stack_top(int)(&s) == i * 3); stl_stack_pop(int)(&s); }
        CHECK(stl_stack_size(int)(&s) == 10);
    }

    /* ---------------- Queue (环形回绕) ---------------- */
    {
        STL_Queue(int) q; stl_queue_init(int)(&q, ar);
        CHECK(stl_queue_empty(int)(&q));
        for (int i = 0; i < 12; i++) stl_queue_push_back(int)(&q, i + 1);  /* 1..12, cap=16 */
        CHECK(stl_queue_size(int)(&q) == 12);
        CHECK(stl_queue_front(int)(&q) == 1);
        CHECK(stl_queue_back(int)(&q) == 12);
        /* 弹 8 个(begin 前移), 再推 6 个触发环形回绕 */
        for (int k = 1; k <= 8; k++) CHECK(stl_queue_pop_front(int)(&q) == k);
        for (int i = 0; i < 6; i++)  stl_queue_push_back(int)(&q, 100 + i);
        CHECK(stl_queue_front(int)(&q) == 9);       /* 环形跨过物理末尾 */
        CHECK(stl_queue_back(int)(&q) == 105);
        for (int k = 9; k <= 12; k++) CHECK(stl_queue_pop_front(int)(&q) == k);
        for (int k = 0; k <= 5; k++)  CHECK(stl_queue_pop_front(int)(&q) == 100 + k);
        CHECK(stl_queue_empty(int)(&q));
        stl_queue_clear(int)(&q);
    }
    ...
    /* ---------------- 用户值类型 operator 驱动 ---------------- */
    {
        ...
        pts[3].x = 1; pts[3].y = 0;
        pts[4].x = 3; pts[4].y = 0;
        stl_sort(struct Pt)(pts, 5);               /* 默认 operator_lt(x) */
        for (int i = 0; i < 5; i++) CHECK(pts[i].x == i + 1);

        struct Pt key = { 1, 0 };
        CHECK(pts[0].x == 1);
        struct Pt *hit = stl_find(struct Pt)(pts, pts + 5, key);  /* operator_eq */
