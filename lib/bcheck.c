...
#elif defined CONFIG_TCC_MUSL
/* tcc_posix musl-only build: no winapi, no pthread/dlfcn.  The runtime is
   linked into a single-threaded musl exe, so the semaphore/TLS layers are
   compiled out (CONFIG_TCC_SEMLOCK=0 philosophy).  bcheck.o itself is
   compiled WITHOUT -b, so its own malloc() calls go straight to musl's real
   malloc (never recursing through __bound_malloc). */
#define INIT_SEM()
#define EXIT_SEM()
#define WAIT_SEM()
#define POST_SEM()
    ...
    if (tree) {                                                                \
        ...
        if (addr <= tree->size) {                                              \
            if (tree->is_invalid || addr + offset + dsize > tree->size) {      \
                POST_SEM ();                                                   \
                if (never_fatal <= 0) {                                        \
                    /* fatal: report once and abort.  Returning INVALID_POINTER \
                       here would let the subsequent real access raise a       \
...
/* tcc_posix: posix_memalign/aligned_alloc are renamed (tccdefs_.h) to these
   __bound_* entries so their aligned blocks also get a bounds region.  bcheck.o
   is compiled WITHOUT -b, so posix_memalign/aligned_alloc below resolve to the
   real musl allocator.  Region bookkeeping matches __bound_memalign. */
int __bound_posix_memalign(void **memptr, size_t align, size_t size)
{
    int ret = posix_memalign(memptr, align, size + 1);
    if (ret == 0 && NO_CHECKING_GET() == 0) {
        WAIT_SEM ();
        INCR_COUNT(bound_memalign_count);
        if (*memptr) {
            tree = splay_insert((size_t) *memptr, size ? size : size + 1, tree);
            if (tree && tree->start == (size_t) *memptr)
                tree->type = TCC_TYPE_MEMALIGN;
        }
        POST_SEM ();
    }
...
void *__bound_aligned_alloc(size_t align, size_t size)
{
    void *ptr = aligned_alloc(align, size + 1);
    if (NO_CHECKING_GET() == 0) {
        WAIT_SEM ();
        INCR_COUNT(bound_memalign_count);
        if (ptr) {
