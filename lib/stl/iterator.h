 ...
 *   model (T) T *stl_vector_data(stl_vector(T) *self);        // begin
 *   model (T) T *stl_vector_end  (stl_vector(T) *self);       // = data + len
 * 遍历: for (T *it = stl_vector_data(int)(&v); it != stl_vector_end(int)(&v); ++it) ...
